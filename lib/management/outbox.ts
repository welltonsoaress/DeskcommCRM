...
export type ManagementOutboxKind = "verification" | "consultation" | "daily" | "alert";
...
export async function enqueueManagementDelivery(admin: Admin, input: {
  ...
  alertSource?: string; referenceId?: string;
}): Promise<boolean> {
  const { error } = await admin.from("management_outbox" as never).insert({
    ...
    body: input.kind === "verification" ? sealVerificationBody(input.body.slice(0, 3000))
      : input.body.slice(0, 3000), status: "pending",
