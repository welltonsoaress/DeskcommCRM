...
>
> O [`hostgator-setup-kit/`](hostgator-setup-kit/) configura o app, o WhatsApp e o banco
> numa VPS com Docker usando um único comando, e o
> [runbook de produção](docs/runbooks/waha-hostgator.md) já assume esse ambiente.
>
> Escolha uma VPS com Docker e confirme preços e condições diretamente com o provedor.
>
...
| Item | Onde conseguir |
|---|---|
| **VPS com Docker** | Qualquer provedor compatível; 4 GB de RAM recomendados |
...
*"instala o Striva Sales pra mim"*. Ele lê o [`CLAUDE.md`](hostgator-setup-kit/CLAUDE.md) do kit
...
**Striva Sales** é um CRM independente com agentes de IA nativos e WhatsApp, criado para acompanhar o trabalho comercial de ponta a ponta.

O sistema atende negócios de vários segmentos, como **clínicas, imobiliárias, infoprodutos, agências, lojas e prestadores de serviço** — qualquer operação que vende pelo WhatsApp. É um **sistema operacional de vendas**: agentes de IA com RAG por tenant atendem, qualificam, movem leads no funil, disparam automações e sabem a hora de passar pra um humano — com o CRM inteiro exposto via **MCP** pros agentes operarem de verdade. A história completa está em [`VISION.md`](VISION.md).
...
| **Hospedagem** | VPS com Docker | App + WhatsApp + workers na sua máquina |
...
Configure os checks obrigatórios para a `main` deste projeto. A lista histórica era do repositório original — **meça a proteção daqui antes de confiar nela**:

```bash
gh api repos/welltonsoaress/striva-sales/branches/main/protection \
  --jq '.required_status_checks.contexts|join(", ")'
# Configure os checks obrigatórios neste repositório; a medição de 2026-08-14 era do projeto original.
```
...
Abra uma [issue](https://github.com/welltonsoaress/striva-sales/issues/new/choose) — o template pede o que precisamos (ambiente, `/api/v1/health`, steps). Rodar `bash hostgator-setup-kit/healthcheck.sh` e colar a saída ajuda muito.

Pra **vulnerabilidades de segurança**, **NÃO abra issue pública** — use o [relato privado de vulnerabilidades](https://github.com/welltonsoaress/striva-sales/security/advisories/new). Detalhes em [`SECURITY.md`](SECURITY.md).
...
- **MCP público** — capabilities do CRM expostas pro ecossistema de agentes: plugue o agente que quiser e ele opera o Striva Sales.
...
- **Discussões:** [GitHub Discussions](https://github.com/welltonsoaress/striva-sales/discussions) — pra perguntas, ideias, showcase.
- **Issues:** [GitHub Issues](https://github.com/welltonsoaress/striva-sales/issues) — bugs e tasks.
- **Créditos de origem:** [@melgarafael](https://www.instagram.com/melgarafael) (perfil pessoal, não canal oficial deste projeto).
...
- **Suporte é comunitário e "as-is".** Dúvidas e bugs entram como
  [Issues](https://github.com/welltonsoaress/striva-sales/issues) ou
  [Discussions](https://github.com/welltonsoaress/striva-sales/discussions). Não há SLA nem
...
- **LGPD — atenção:** quem **hospeda** a instância é o **controlador** dos dados pessoais
  ...
  têm acesso ao seu banco, ao seu WhatsApp nem ao seu storage. A telemetria abaixo
  fica desligada até você configurar sua própria conta.
- **Telemetria (Sentry):** neste projeto ela fica **desligada por padrão**, inclusive
  se `SENTRY_DSN` estiver vazio. Para ativar, configure `SENTRY_DSN=<seu-dsn>`
  no `.env`; para desligar, use `SENTRY_DSN=off`. Os hooks de higienização estão em
...
- **HostGator** — parceria documentada pelo projeto original para o kit de instalação.
